describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const prodItems = await page.$$("product-item");
    var button = await prodItems[0].getProperty("shadowRoot");
    button = await button.$("button");
    await button.click();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let i = 1; i < prodItems.length; i++){
      // first button is already clicked above
      var button = await prodItems[i].getProperty("shadowRoot");
      button = await button.$("button");
      await button.click();
    }
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const local = await page.evaluate(() => localStorage.cart )
